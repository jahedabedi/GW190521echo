...
if EchoSNR == 1:
    ...
    static_args = {'approximant': 'TDBechoes3', 'imr_approximant': 'NRSur7dq4', 'n_echoes': 2, 'alpha': 1, 'include_main_event': True, 'f_lower': 20.0,'f_ref': 20.0}
    ...
    for ifo,d in {'H1':strain_H1, 'L1':strain_L1, 'V1':strain_V1}.items():
        ...
        #print(dtilde[100000],dtilde[500000])
        htilde.resize(len(dtilde))
        optimal += np.array(htilde)*(np.array(dtilde).conjugate())/( sum(abs(dtilde)**1) )
        sigmasq += sum( np.array(htilde)*(np.array(htilde).conjugate())/( sum(abs(dtilde)**1)) )
    ...
    print(cplx_hd)
    print(hh)
    print('SNR=',sum(abs(optimal))/(sigmasq**0.5))
    print('SNR=',max(   abs(optimal)/(sigmasq**0.5)  ) )
    print('SNR=', sum(optimal/(sigmasq**0.5)) )
...
mins = {'del_t_echo':-0.5, 'horizon_freq':25}
maxs = {'del_t_echo':3.5, 'horizon_freq':80}
...
fig.legend(loc=(0.27,0.6), ncol=1, handles=handles1, labels=['Maximum likelihood Boltzmann echo waveform', 'pixel SNR = 0.5', 'pixel SNR = 4.9'])
ax.axis([-0.5, 3.5, 25, 80])
...
#ax.set_xlabel('$t-t_{\\rm{merger}}$ [sec]')
#ax.set_ylabel('Frequency [Hz]')
fig.legend(loc=(0.45,0.83), ncol=2, handles=handles2, labels=['Predicted Planckian echoes'])

fig.savefig('Xpec_PyCBC_GR_NRSur.pdf')
